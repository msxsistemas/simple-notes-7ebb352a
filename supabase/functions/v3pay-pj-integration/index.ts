...
async function handleWebhook(body: any, supabaseAdmin: any) {
  ...
  console.log('📩 V3Pay PJ Webhook received:', JSON.stringify(body).substring(0, 500));
  ...
  if (event !== 'order.paid') {
    console.log(`ℹ️ V3Pay PJ event "${event}" - acknowledged`);
    return new Response(JSON.stringify({ success: true, event }),
      { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
  }

  // order.paid — process payment (Woovi pattern)
  ...
  if (!chargeId || chargeId.length < 1) {
    console.warn('⚠️ V3Pay PJ webhook: missing charge ID');
  ...
  if (!cobranca && !fatura) {
    console.warn(`V3Pay PJ webhook: no matching records for chargeId: ${chargeId}`);
  ...
  // Verify user has V3Pay PJ configured
  const { data: v3Config } = await supabaseAdmin
    .from('v3pay_pj_config')
    .select('id')
  ...
  if (!v3Config) {
    console.warn(`V3Pay PJ webhook: no active config for user ${ownerUserId}`);
  ...
  if (fatura && fatura.status !== 'pago') {
    ...
    if (updatedFatura) {
      shouldTriggerAutoRenew = true;
      console.log(`✅ Fatura ${fatura.id} marked as paid via V3Pay PJ webhook`);
    }
  }
  ...
  if (shouldTriggerAutoRenew && ownerUserId && clienteWhatsapp) {
    ...
    console.log(`V3Pay PJ webhook: charge ${chargeId} already processed`);
  }
  ...
  switch (action) {
    case 'configure': {
      ...
      try {
        ...
        console.log('✅ V3Pay PJ token validated, status:', testResp.status);
      } catch (error: any) {
      ...
      await storeVaultSecret(userToken || '', user.id, 'v3pay_pj', 'api_token', apiToken);
      ...
      const { error: upsertError } = await supabaseAdmin
        ...
        .upsert({
          ...
          is_configured: true,
          webhook_url: `${Deno.env.get('SUPABASE_URL')}/functions/v1/v3pay-pj-integration`,
        }, { onConflict: 'user_id' });
      ...
      return new Response(JSON.stringify({ success: true, message: 'V3Pay PJ configurado com sucesso!' }),
        { headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }

    case 'create-charge': {
      const apiToken = await getVaultSecret(supabaseAdmin, user.id, 'v3pay_pj', 'api_token');

      if (!apiToken) {
        return new Response(JSON.stringify({ success: false, error: 'V3Pay PJ não configurado.' }),
          { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 400 });
      }
...
Deno.serve(async (req) => {
  ...
  try {
    console.log('🚀 V3Pay PJ Integration - Starting request processing...');
    ...
    console.error('🚨 V3Pay PJ Error:', error);
